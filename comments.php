...
<?php if ( get_comment_type() == "comment" ) : ?>
                    <li id="comment-<?php comment_ID() ?>" class="<?php sandbox_comment_class() ?>">
                        <div class="comment-author vcard"><?php alicitron_commenter_link() ?></div>
                        <div class="comment-meta"><?php printf(__('<a href="%3$s" title="Permalink to this comment">%1$s, at %2$s</a>', 'sandbox'),
                                    get_comment_date(),
                                    get_comment_time(),
...
<?php $sandbox_comment_alt = 0 ?>
                ...
                <ul id="trackbacks-list" class="comments">
<?php foreach ( $comments as $comment ) : ?>
<?php if ( get_comment_type() != "comment" ) : ?>
...
<?php $req = get_option('require_name_email'); // Checks if fields are required. Thanks, Adam. ;-) ?>
                ...
                <section id="respond">
                    <h2><?php _e( 'Leave a Reply', 'sandbox' ) ?></h2>
